...
- 👥 **用户资源** — 各用户资源占用情况
- 🌐 **在线终端** — 当前在线用户终端信息
- 📌 **更新策略** — 顶部信息栏展示数据刷新节奏（日间 08:00–24:00 每 20 分钟生成，夜间 00:00–08:00 每 2 小时生成，页面每 5 分钟轮询核查）
...
- **推送**：`sync.sh` 通过 GitHub Contents API 更新 `data/slurm.json`；由 cron 驱动——日间（08:00–24:00）每 20 分钟一次，夜间（00:00–08:00）每 2 小时一次。
